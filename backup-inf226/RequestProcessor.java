...
public final class RequestProcessor extends Thread {
    ...
    private final HashMap<InetAddress, Integer> connectedHostsLog;

    public RequestProcessor() {
        queue = new LinkedBlockingQueue<Request>();
        connectedHostsLog = new HashMap<>();
        HostAccessTimer hostAccessTimer = new HostAccessTimer();
        hostAccessTimer.start();
    }
    ...
    public void run() {
        try {
            ...
            while (true) {
                ...
                InetAddress hostAddress = request.client.getInetAddress();
                //System.out.println("Host " + hostAddress + " is requesting operation");

                if (connectedHostsLog.get(hostAddress) == null) {
                    connectedHostsLog.put(hostAddress, 0);
                    //System.out.println("Added new host to list of connected hosts");
                }
                if (connectedHostsLog.get(hostAddress) > 10) {

                    final BufferedWriter out =
                            new BufferedWriter(new OutputStreamWriter(request.client.getOutputStream()));
                    out.write("YOU HAVE MADE TOO MANY REQUESTS. PLEASE COME BACK LATER\n");
                    out.flush();
                    out.close();
    ...
    private void incrementAttempt(InetAddress host) {
        for (Map.Entry<InetAddress, Integer> entry : connectedHostsLog.entrySet()) {
            if (entry.getKey().toString().equals(host.toString())) {
                entry.setValue(entry.getValue() + 1);
                //System.out.println("INCREMENT: Host  " + host + " has " + entry.getValue() + " attempts");
                return;
            }
        }
    }
    ...
    private void decrementAttempts() {
        for (Map.Entry<InetAddress, Integer> entry : connectedHostsLog.entrySet()) {
            if (entry.getValue() > 0) {
                entry.setValue(entry.getValue() - 1);
                //System.out.println("DECREMENT: Host " + entry.getKey() + " now has " + entry.getValue() + " attempts");
            }
        }
    }

    class HostAccessTimer extends Thread {
